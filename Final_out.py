...
# Set up logging
logging.basicConfig(filename='longest_depth_files.log', level=logging.INFO, 
...
# Function to find the top 5 files with the longest depth
def find_top_5_longest_depth(directory):
    files_with_depths = []
...
# Function to read the top 5 files from longest_depth_files.log
...
# Directories for twitter15 and twitter16
directories = {
    'twitter15': './processed_data15/false_trees',  # Update this to the correct path if necessary
    'twitter16': './processed_data16/false_trees'   # Update this to the correct path if necessary
}

# Read tweet descriptions
tweet_descriptions_files = {
    'twitter15': './processed_data15/false_source_tweets.txt',  # Update this to the correct path if necessary
    'twitter16': './processed_data16/false_source_tweets.txt'   # Update this to the correct path if necessary
}

# Find and log the top 5 files with the longest depth and plot the longest path for each directory
for key, directory in directories.items():
    top_5_files = find_top_5_longest_depth(directory)
    tweet_descriptions_file_path = tweet_descriptions_files[key]
    matched_descriptions = match_descriptions('longest_depth_files.log', tweet_descriptions_file_path)
    logging.info(f"Top 5 files with the longest depth in {key}:")
    for file, depth in top_5_files:
...
print("Top 5 files with the longest depth logged in 'longest_depth_files.log' and plots created.")
